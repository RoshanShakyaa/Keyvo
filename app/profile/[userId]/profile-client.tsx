...
import { getRaceStatsQueryOptions, getRecentRacesQueryOptions, getRecentTestsQueryOptions, getUserStatsQueryOptions } from "@/hooks/useQueryOptions";
...
export function ProfileClient({ user, isOwnProfile }: ProfileClientProps) {
  // Viewing someone else's profile → staler data is fine (60s)
  // Viewing your own profile → keep it fresh (15s, refetch on focus)
  const staleTime = isOwnProfile ? 15_000 : 60_000;

  const { data: statsRes } = useQuery({
    ...getUserStatsQueryOptions(user.id),
    staleTime,
    refetchOnWindowFocus: isOwnProfile,
  });
  const { data: testsRes } = useQuery({
    ...getRecentTestsQueryOptions(user.id),
    staleTime,
    refetchOnWindowFocus: isOwnProfile,
  });
  const { data: racesRes } = useQuery({
    ...getRecentRacesQueryOptions(user.id),
    staleTime,
    refetchOnWindowFocus: isOwnProfile,
  });
  const { data: raceStatsRes } = useQuery({
    ...getRaceStatsQueryOptions(user.id),
    staleTime,
    refetchOnWindowFocus: isOwnProfile,
  });
